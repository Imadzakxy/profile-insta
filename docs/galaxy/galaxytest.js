...
function createStars() {
    ...
    for (let arm = 0; arm < config.spiralArms; arm++) {
        const baseAngle = arm * config.armSpacing * Math.PI / 180;
        for (let i = 0; i < config.pointsPerArm; i++) {
            const progress = i / config.pointsPerArm;
            const radius = progress * config.maxRadius * (1 + (Math.random() - 0.5) * 0.1);
            const angleOffset = (Math.random() - 0.5) * config.armWidth * progress * Math.PI / 180;
            const brightness = 100 + Math.random() * 120;
            stars.push({
                baseAngle,
                spiralAngle: progress * Math.PI * 2 * config.spiralTurns,
                progress,
                radius,
                angleOffset,
                brightness,
...
function updateStars(deltaMs) {
    ...
    for (const star of stars) {
        ...
        if (star.radius <= config.minRadius) {
            ...
            star.angleOffset = (Math.random() - 0.5) * config.armWidth * star.progress * Math.PI / 180;
        }

        star.angle = star.baseAngle + star.spiralAngle + rotationRad + star.angleOffset * config.randomness;
    }
}
